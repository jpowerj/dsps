...
* The book uses the term **probability function**, and the abbreviation **p.f.**, to refer to the function $p_X(v)$ that gives us the probability that a discrete random variable $X$ takes on the value $v$. To match the notation we use in class, I have changed this throughout to use the term **probability mass function**, and the abbreviation **pmf**.
