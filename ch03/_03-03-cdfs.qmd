...
Suppose that a random variable $X$ has the Bernoulli
distribution with parameter $p = 0.7$. (See @def-3-1-5) Sketch the CDF of $X$.
...
![Figure 3.9: The CDF for Exercise 4.](ch03/images/fig-3-9.jpeg){#fig-3-9}

::: {#exr-3-3-5}

# Exercise 3.3.5

Suppose that the CDF of a random variable $X$ is as
follows:
...
Suppose that $X$ has the uniform distribution on the
interval $[0, 5]$ and that the random variable $Y$ is defined by $Y = 0$ if $X \leq 1$, $Y = 5$ if $X \geq 3$, and $Y = X$ otherwise. Sketch the CDF of $Y$.
...
Find the quartiles and the median of the binomial
...
Let $X$ be a random variable with quantile function
$F^{-1}$. Assume the following three conditions: (i) $F^{-1}(p) = c$ for all $p$ in the interval $(p_0, p_1)$, (ii) either $p_0 = 0$ or $F^{-1}(p_0) < c$, and (iii) either $p_1 = 1$ or $F^{-1}(p) > c$ for $p > p_1$. Prove that $\Pr(X = c) = p_1 − p_0$.
